...
describe ICS do
  describe 'parse_date' do
    let(:date){ ActiveSupport::TimeZone["UTC"].parse("2015-10-14T12:00:00Z") }
    context "UTC" do
      let(:line){ {"DTSTART;TZID=UTC" => "20151014T120000"} }
      it { expect(ICS::parse_date(line, 'DTSTART')).to eq(date) }
    end
    context "Asia/Tokyo" do
      let(:line){ {"DTSTART;TZID=Asia/Tokyo" => "20151014T210000"} }
      it { expect(ICS::parse_date(line, 'DTSTART')).to eq(date) }
    end
    context "Asia/Tokyo" do
      let(:line){ {"DTSTART;TZID=America/Los_Angeles" => "20151014T050000"} }
      it { expect(ICS::parse_date(line, 'DTSTART')).to eq(date) }
    end
  end
