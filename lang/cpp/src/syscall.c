...
channel_t sys_open(void) {
    ...
    if (do_syscall(SYSCALL_OPEN, 0, 0, 0, 0, &r) != OK) {
...
result_t sys_close(channel_t ch) {
    uintmax_t r;
    return do_syscall(SYSCALL_CLOSE, ch, 0, 0, 0, &r);
}


result_t sys_wait(channel_t ch) {
    uintmax_t r;
    return do_syscall(SYSCALL_WAIT, ch, 0, 0, 0, &r);
}
...
result_t sys_setoptions(channel_t ch, handler_t handler, void *buffer,
    ...
    return do_syscall(SYSCALL_SETOPTIONS, ch, (uintmax_t) handler,
	    (uintmax_t) buffer, (uintmax_t) size, &r);
}


result_t sys_send(channel_t ch, payload_t *m) {
    uintmax_t r;
    return do_syscall(SYSCALL_SEND, ch, (uintmax_t) m, 0, 0, &r);
}
...
payload_t *sys_recv(channel_t ch) {
    ...
    if (do_syscall(SYSCALL_RECV, ch, 0, 0, 0, &r) != OK) {
        // TODO: implement something like errno
        return 0;
    }
...
result_t sys_call(channel_t ch, payload_t *m, void *buffer, size_t buffer_size) {
    uintmax_t r;
    return do_syscall(SYSCALL_CALL, ch, (uintmax_t) m, (uintmax_t) buffer,
                      (uintmax_t) buffer_size, &r);
}


result_t sys_link(channel_t ch1, channel_t ch2) {
    uintmax_t r;
    return do_syscall(SYSCALL_LINK, ch1, ch2, 0, 0, &r);
}


result_t sys_transfer(channel_t ch1, channel_t ch2) {
    uintmax_t r;
    return do_syscall(SYSCALL_TRANSFER, ch1, ch2, 0, 0, &r);
}
