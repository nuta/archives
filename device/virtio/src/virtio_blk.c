...
result_t virtio_blk_write(uintmax_t sector, size_t n, const void *data) {
    ...
    header = allocPhysicalMemory(0, sizeof(*header), ALLOCMEM_CONTINUOUS,
                                 &header_paddr);
    ...
    buf = allocPhysicalMemory(0, data_size, ALLOCMEM_CONTINUOUS, &paddr);
    ...
    allocPhysicalMemory(0, sizeof(*status), ALLOCMEM_CONTINUOUS, &status_paddr);
