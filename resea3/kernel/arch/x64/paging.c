...
static void lookup_page_entry(struct arch_vmspace *vms, uptr_t v, bool allocate,
    ...
    for (int i = 4; i > 1; i--) {
        ...
        if (!entries[idx]) {
            ...
            if (allocate) {
                paddr_t paddr;
                paddr = alloc_pages(LEN_TO_PAGE_NUM(PAGE_SIZE), KMALLOC_NORMAL);
...
void arch_create_vmspace(struct arch_vmspace *vms) {
    ...
    paddr_t pml4_addr = alloc_pages(LEN_TO_PAGE_NUM(pml4_size), KMALLOC_NORMAL);
...
void x64_init_paging(void) {
    ...
    paddr_t pml4_addr = alloc_pages(LEN_TO_PAGE_NUM(PAGE_SIZE), KMALLOC_NORMAL);
    kernel_pml4 = from_paddr(pml4_addr);
    memset((void *) kernel_pml4, 0, PAGE_SIZE);

    paddr_t pdpt_addr = alloc_pages(LEN_TO_PAGE_NUM(PAGE_SIZE), KMALLOC_NORMAL);
    u64_t *pdpt = from_paddr(pdpt_addr);
    memset((void *) pdpt, 0, PAGE_SIZE);

    for (u64_t i = 0; i < PAGE_ENTRY_NUM; i++) {
        paddr_t pd_addr = alloc_pages(LEN_TO_PAGE_NUM(PAGE_SIZE), KMALLOC_NORMAL);
