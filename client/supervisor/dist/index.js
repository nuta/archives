...
const http_adapter_1 = require("./adapters/http_adapter");
const sakuraio_adapter_1 = require("./adapters/sakuraio_adapter");
...
class Supervisor {
    constructor({ adapter, appDir, deviceType, osType, osVersion, deviceId, deviceSecret, debugMode, appUID, appGID, heartbeatInterval }) {
        this.app = null;
        this.appDir = appDir;
        this.osType = osType;
        ...
        switch (this.adapterName) {
            case 'http':
                this.adapter = new http_adapter_1.default(this.osType, this.deviceId, adapter.url);
                this.verifyHMAC = true;
                this.includeHMAC = true;
                this.includeDeviceId = true;
                break;
            case 'sakuraio':
                this.adapter = new sakuraio_adapter_1.default();
    ...
    launchApp(appZip) {
        ...
        child_process_1.spawnSync('rm', ['-rf', path.join(this.appDir)], {
            stdio: 'inherit'
        });
        child_process_1.spawnSync('mkdir', ['-p', this.appDir], {
            stdio: 'inherit'
        });
        child_process_1.spawnSync('unzip', ['-q', appZipPath, '-d', this.appDir], {
            stdio: 'inherit'
        });
        this.spawnApp();
    }
    ...
    doSpawnApp() {
        this.app = child_process_1.fork('./start', [], {
            cwd: this.appDir,
            stdio: 'inherit',
            uid: this.appUID,
            gid: this.appGID,
    ...
    async start() {
        ...
        this.adapter.onReceive(payload => {
            ...
            if (this.verifyHMAC && !this.verifyMessageHMAC(hmacProtectedPayload, messages.hmac, messages.timestamp)) {
